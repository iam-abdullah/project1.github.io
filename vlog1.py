...
def contact():
    if(request.method=='POST'):
        ...
        entry = Contacts(name=name, phone_no = phone, mes = message, date= datetime.now(),email = email )
...
def edit(sno):
    if "user" in session and session['user'] == params['admin_user']:
        if request.method == "POST":
            ...
            if sno == '0':
                post = Posts1(title= box_title, slug=slug, tagline=tline, content=content, date=date, img_file=img_file)
                db.session.add(post)
                db.session.commit()
            else:
